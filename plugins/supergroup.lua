local function check_member_super(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '<b>After this, the group protects</b>'
...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '<b>To me, there is nothing</b>'
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local title ="<b>Info for SuperGroup: ["..result.title.."]</b>\n\n"
local admin_num = "<b>Admin count: "..result.admins_count.."</b>\n"
local user_num = "<b>User count: "..result.participants_count.."</b>\n"
local kicked_num = "<b>Kicked user count: "..result.kicked_count.."</b>\n"
local channel_id = "<b>ID: "..result.peer_id.."</b>\n"
...
local function lock_group_membermod(msg, data, target)
  ...
  return '<i>SuperGroup</i> <b>members</b> has been <b>locked</b>'
end
...
local function unlock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'no' then
    return '<b>SuperGroup members</b> are not <b>locked</b>'
  else
		...
		local action = msg.action.type
			...
			if action == 'chat_add_user' then
				...
				if is_support(msg.action.user.id) and not is_owner2(msg.action.user.id) then
					local receiver = get_receiver(msg)
					local user = "user#id"..msg.action.user.id
					savelog(msg.to.id, name_log.." <b>Support member ["..msg.action.user.id.."] added to the SuperGroup by [ "..msg.from.id.." ]</b>")
					channel_set_mod(receiver, user, ok_cb, false)
				end
			end
		end
...
local function pre_procses(msg)
